...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('book_to_category'))) {
    await knex.schema.createTable('book_to_category', table => {
      table.increments('id')
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.dropTableIfExists('book_to_category')
